...
export default function LibraryPage({ likedTracks }: { likedTracks: LikedTrack[] }) {
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 p-4 sm:p-6">
      ...
      {likedTracks.length === 0 ? (
        <p className="text-sm text-white/40">
          Vous n'avez encore aimé aucun titre. Cherchez de la musique et cliquez sur le cœur pour
          l'ajouter ici.
        </p>
      ) : (
          ...
          {likedTracks.map((track) => (
            <li key={track.id} className="flex items-center gap-3 py-2.5">
              <div className="relative h-11 w-11 shrink-0 overflow-hidden rounded-md bg-surface-elevated">
                <Image src={track.albumCover} alt="" fill sizes="44px" className="object-cover" />
              </div>
